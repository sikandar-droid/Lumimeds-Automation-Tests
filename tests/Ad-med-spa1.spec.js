...
const AdPage = require('./pages/Ad-med-spa1');
...
// Ad pages to test
const adPages = [
    { name: 'med-spa1', title: 'Stop overpaying for weight loss care' },
];
    ...
    for (const adPageInfo of adPages) {
        test.describe(`Testing: /ad/${adPageInfo.name}`, () => {
            ...
            test('should verify Choose Your Plan Now button navigates to survey', async ({ page }) => {
                ...
                console.log('\n🔍 Testing "Choose Your Plan Now" button navigation...');

                // Check if Choose Your Plan Now button exists
                const chooseButtonVisible = await adPage.chooseYourPlanButton.first().isVisible({ timeout: 5000 }).catch(() => false);
                const chooseLinkVisible = await adPage.chooseYourPlanLink.first().isVisible({ timeout: 5000 }).catch(() => false);
                
                expect(chooseButtonVisible || chooseLinkVisible).toBeTruthy();
                console.log('✅ Found "Choose Your Plan Now" button');
                ...
                try {
                    const button = chooseButtonVisible ? adPage.chooseYourPlanButton.first() : adPage.chooseYourPlanLink.first();
                    ...
                    expect(isCorrectUrl).toBeTruthy();
                    console.log(`✅ "Choose Your Plan Now" correctly navigates to survey page`);
                } catch (e) {
                    console.log(`⚠️ Navigation test failed: ${e.message}`);
                    throw e;
                }
