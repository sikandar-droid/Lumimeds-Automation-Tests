...
class AdPage {
    constructor(page) {
        ...
        this.startYourJourneyButton = page.getByRole('button', { name: /start your journey now/i });
        this.startYourJourneyLink = page.getByRole('link', { name: /start your journey now/i });
    ...
    /**
     * Get all CTA buttons that navigate to survey (Get Started, Choose Your Plan Now, Start Now, Start Your Journey Now)
     */
    async getAllGetStartedButtons() {
        ...
        const journeyButtons = await this.startYourJourneyButton.all();
        const journeyLinks = await this.startYourJourneyLink.all();
        return [...buttons, ...links, ...chooseButtons, ...chooseLinks, ...startButtons, ...startLinks, ...journeyButtons, ...journeyLinks];
    }

    /**
     * Click the primary CTA button (Get Started, Start Now, Start Your Journey Now, or Choose Your Plan Now)
     */
    async clickPrimaryGetStarted() {
        // Priority order: Try the most visible/prominent button first
        
        // Try regular "Get Started" button first (most common)
        try {
            if (await this.primaryCTAButton.isVisible({ timeout: 3000 })) {
            ...
            // Button not found, try alternatives
        }
        
        // Try "Start Your Journey Now" button (for thanksgiving page)
        try {
            if (await this.startYourJourneyButton.isVisible({ timeout: 3000 })) {
                await this.startYourJourneyButton.first().scrollIntoViewIfNeeded();
                await this.page.waitForTimeout(1000);
                await this.startYourJourneyButton.first().click();
                return;
            }
        } catch (e) {
        ...
        try {
            if (await this.startYourJourneyLink.isVisible({ timeout: 3000 })) {
                await this.startYourJourneyLink.first().scrollIntoViewIfNeeded();
                await this.page.waitForTimeout(1000);
                await this.startYourJourneyLink.first().click();
                return;
            }
        } catch (e) {
        ...
        // Try "Start Now" button (for best-weight-loss-medication page)
        try {
        ...
        try {
            ...
            throw new Error('No CTA button found on the page');
        }
    }
